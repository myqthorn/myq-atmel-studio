...
#define SPI_SETTING ((1<<SPE) | (0<<SPIE)| (1<<DORD)| (1<<MSTR)| (1<<SPR1)|(0<<SPR0)| (0<<CPOL)| (0<<CPHA))            
...
void NRF::process(void){
	...
	if( isInitializing() && !hasDataToSend() && !waitingForResponse() ) {
		...
		status |= ((1<<NRF_TX_READY) | (1<<NRF_WAIT_FOR_RESPONSE));
		
		initCount++;
		////after the last setup message, clear the init flag in the status register
	...
	if (!isReadyToReceiveData()){
		...
		//prepare RxData to receive again
		//clearRxData();		
...
void NRF::processCommandResponse(){
	...
	switch (RxData[2]){	//CommandOpCode
		...
		case NRF_CMD_SETUP:		//pg 102
			...
			//rerun setup upon failure -- this doesn't work
			//if (RxData[3]==ACI_STATUS_ERROR_DEVICE_STATE_INVALID)
			//status |= (1<<NRF_INIT_MODE);
...
uint8_t NRF::PrepareTxData(uint8_t command){
	return (PrepareTxData(command, 0,  0));
}
...
void NRF::transferACI(void){
	...
	//if the Rx LENGTH BYTE is larger than the remaining length, 
	//set length to the Rx LENGTH BYTE
	if (RxData[0]>length)
		length = RxData[0];
